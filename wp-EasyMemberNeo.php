<?php
/*
Plugin Name: wp-EasyMemberNeo!
Plugin URI: http://www.easymemberneo.com
Description: An Extension to Easy Member Neo, Allowing for the Viewing of Pages and Posts Based on Membership Levels, and drip feed options.
Version: 1.2.3
Author: EasyMemberNeo
Author URI: http://www.easymemberneo.com
*/
	if ( ! defined( 'ABSPATH' ) ) exit;
	class wp_EasyMemberNeo
	{
		var $message;
		var $err;
		...
		{
			if ( ! defined( 'WP_PLUGIN_URL' ) 	) 	define( 'WP_PLUGIN_URL'		, plugin_dir_url( __FILE__ ) );
			...
			add_filter( 'comments_template'		, array( &$this, 'wpemn_comments_template'));
			...
			add_shortcode('emn-member-profile',array(&$this,'wpemp_display_profile'));
			add_shortcode('emn-member-memberships',array(&$this,'wpemp_display_memberships'));
			
			add_shortcode('emn-firstname',array(&$this,'wpemp_firstname'));
			add_shortcode('emn-lastname',array(&$this,'wpemp_lastname'));
			add_shortcode('emn-email',array(&$this,'wpemp_email'));
			add_shortcode('emn-addr',array(&$this,'wpemp_addr'));
			add_shortcode('emn-addr2',array(&$this,'wpemp_addr2'));
			add_shortcode('emn-addr3',array(&$this,'wpemp_addr3'));
			add_shortcode('emn-city',array(&$this,'wpemp_city'));
			add_shortcode('emn-state',array(&$this,'wpemp_state'));
			add_shortcode('emn-zipcode',array(&$this,'wpemp_zipcode'));
			add_shortcode('emn-country',array(&$this,'wpemp_country'));
			add_shortcode('emn-telephone',array(&$this,'wpemp_telephone'));
			add_shortcode('emn-mobile',array(&$this,'wpemp_mobile'));	
		}
		...
		function wpemp_style() {
			$myStyleUrl 	= WPEMP_URL . 'css/wp-easymemberneo.css';
			$myStyleFile 	= WPEMP_DIR . 'css/wp-easymemberneo.css';
		...
		function wpemp_script(){
			wp_enqueue_script('wpemp_script', WPEMP_URL . 'js/wp-easymemberneo.js', array('jquery'));
		}
		...
		function wpemp_options_page(){
			...
			<?php wp_nonce_field('wpemn-update-options'); ?>
	...
	function addInput(divName){
			  ...
			  counter++;
				...
				<table class="form-table">
					...
					<tr valign="top">
					<th scope="row">EasyMemberNeo URL</th>
					...
					<span class="description">Please enter the URL to your EasyMemberNeo Installation</span>
					</td>
					</tr>
					<tr valign="top">
					...
					<span class="description">"Powered By EasyMemberNeo" link will be your affiliate link, earning you 65% of all sales you refer to us.<br/>
					...
					<span class="description">"Become a Member" link can be a link to your sale page. Leave it blank to link to EasyMemberNeo Sign up page.</span>
					</td>
					</tr>
	...
	check_admin_referer('wpemn-update-options');
		...
		function wpemp_admin_menu() {
			add_options_page('WP-EasyMemberNeo', 'WP-EasyMemberNeo', 8, 'wpemp_options', array( &$this, 'wpemp_options_page') );
		}
	...
	<table border="0" width="95%" style="text-align:left">
		...
		<tr>
			<td>&raquo;
				<label for="wpemp_visibility"><?php echo  __("Set Page visibility with wp-EasyMemberNeo")  ?></label>
				<div id="levelsLabel" style=" <?php echo $display2 ?>">&raquo;&raquo;
					<label for="wpemp_levels"><?php echo  __("Membership Level Required")  ?></label>
				</div></td>
			<td colspan="2">
				<select name="wpemp_dropdown" id="wpemp_dropdown" onChange= "showLevels(this,'allLevels')">
				<?php
			...
			if($levelcount > 0){
				foreach($levelarray as $v){
					...
					<tr>
						<td><input name="wpemp_levels[]" type="checkbox" <?php echo $checked ?> value="<?php echo $level[0] ?>"/></td>
						<td><?php echo $level[1] ?>-> Membership Days Required</td>
	...
	<?php
		...
		function wpemp_add_custom_box() {
			if( function_exists( 'add_meta_box' )) {
				foreach (array('post','page') as $type) 
					add_meta_box( 'wpemp_section', __( 'wp-EasyMemberNeo Options'), array( &$this, 'wpemp_'.$type.'_custom_box'), $type, 'normal', 'high' );
			}
		}
		...
		function wpemp_content($content = ''){
			...
			if(get_option('wpemp_url') == '' && (get_option('wpemp_show_post') == 'yes' || get_option('wpemp_show_page') == 'yes'))
				return $content . '<div id="wpemp_message"><strong>'.__('EasyMemberNeo URL Required').'</strong><br/>'.__('Please goto settings page and set the URL to EasyMemberNeo').'</div>';
			...
			if($post_option == 'yes' && is_feed() )
			{
				...
				if(get_option('wpemp_show_powered') == 'yes')
					$html .= '<br/><br/>Powered by EasyMemberNeo';
				return $html;
			}
		...
		function wpemp_get_login_link(){
			...
			if(get_option('wpemp_show_powered') == 'yes')
			{
				...
				$html .= ' title="Powered by EasyMemberNeo">EasyMemberNeo</a></span>'."\n";
			}
		...
		function wpemp_get_login_box($post_id){
			...
			<?php if(get_option('wpemp_show_powered') == 'yes'){ ?>
				<span class="wpemp_small">Powered By <a href="'.$this->wpemp_get_aff_link().'" ';
				 target="_blank" title="Powered by EasyMemberNeo">EasyMemberNeo</a></span>
			<?php } ?>
			</p>
			</div>
		...
		function wpemp_get_upgrade_box($post_id){
			...
			if(get_option('wpemp_show_powered') == 'yes')
			{
				...
				$html .= ' target="_blank" title="Powered by EasyMemberNeo">EasyMemberNeo</a></span>'."\n";
			}
			$html .= '</p>'."\n";
			$html .= '</div>'."\n";
		...
		function wpemp_get_aff_link(){
			if($link = get_option("wpemp_powered_link"))
				...
				return 'http://www.easymemberneo.com/?utm_source=blog&utm_medium=blog&utm_campaign=blogpoweredby';
		}
		...
		function wpemp_login(){
			...
			if(isset($_REQUEST['_ajax_nonce']))
			{
				...
				wp_die("Please goto the home page and try login in.", get_bloginfo( 'name' ) . " &raquo; EasyMemberNeo");
			}
			...
			if($wpemp_user == '' || $wpemp_pass == '')
			{
				if(defined('DOING_AJAX'))
					...
					wp_die("Username and Password are required", get_bloginfo( 'name' ) . " &raquo; EasyMemberNeo");
			}
		
			ini_set('allow_url_fopen','1');
			$wmemp_auth_url  = trim(get_option('wpemp_url'), '/').'/api/remoteWP.php?username='.$wpemp_user.'&password='.$wpemp_pass;
		...
		//$auth_res = $result;
			...
			if($auth_res == '0')
			{
				...
				if(defined('DOING_AJAX'))
					...
					wp_die("Username or password incorrect", get_bloginfo( 'name' ) . " &raquo; EasyMemberNeo");
			}
			else
			{
				session_start();
		...
		function wpemn_comments_template($attr){
			if(!isset( $_SESSION['user']['email'] ) )
				$attr = WPEMP_DIR . "wpemn_comments.php";
			return $attr;
		}
	...
	<h3><?php echo _e("EMN Membership Requirements"); ?></h2>
	...
	$html = "";
			...
			$html .= '<tr><td> <label for="wpemp_visibility">' . __("Set Category Visibility with wp-EasyMemberNeo") . '</label>
	...
	$wp_EasyMemberNeo = new wp_EasyMemberNeo();